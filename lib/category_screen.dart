...
import 'package:image_picker/image_picker.dart';
import 'category_model.dart';
import 'record_model.dart';         // RecordItem မော်ဒယ်ကို သုံးရန် Import လုပ်ပါ
import 'record_history_screen.dart'; // History Screen ဆီ သွားရန် Import လုပ်ပါ

// စာရင်းဒေတာများကို App တစ်ခုလုံးမှာ Dynamic သိမ်းထားနိုင်ရန် Global List ကြေညာခြင်း
...
class _CategoryScreenState extends State<CategoryScreen> {
  CategoryState _currentState = CategoryState.view;
  int _currentTabIndex = 2; // Default Category Icon
  ...
  void _onKeypadPressed(String value) {
    setState(() {
      if (value == "⌫") {
        ...
        // --- အမှန်ခြစ် နှိပ်လိုက်လျှင် RecordItem ထဲသို့ Dynamic စာရင်းသွင်းခြင်း ---
        ...
        if (parsedAmount > 0 && _selectedCategory != null) {
          ...
          // Note ဖြည့်ထားရင် Note ပြပြီး၊ ဘာမှမထည့်ထားရင် ၎င်းရဲ့ Category Name ကို ယူပါမည်
          String txTitle = _noteController.text.trim().isNotEmpty 
              ? _noteController.text.trim() 
              : _selectedCategory!.name;

          // 'attach_money' Icon သို့မဟုတ် နာမည်တွင် 'income' ပါက ဝင်ငွေအဖြစ် သတ်မှတ်ခြင်း
          String txType = 'expense';
          if (_selectedCategory!.icon == Icons.attach_money || 
              _selectedCategory!.name.toLowerCase().contains('income')) {
            txType = 'income';
          }
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          trailing: IconButton(
            ...
            onPressed: () => setState(() {
              _itemToModify = item;
              _nameController.text = item.name; // မူရင်းနာမည်ကို စာရိုက်ကွက်ထဲ ထည့်ပေးခြင်း
              _tempColor = item.color;          // မူရင်းအရောင်ကို ရွေးချယ်ပေးထားခြင်း
              _tempIcon = item.icon;            // မူရင်း Icon ကို မှတ်သားပေးခြင်း
              _currentState = CategoryState.edit;
            }),
          ...
          _buildSaveButton(
            ...
            onPressed: () => setState(() {
              if (_nameController.text.isNotEmpty && _itemToModify != null) {
                // --- မူရင်း List ထဲမှာ ပြင်ဆင်မယ့် Item ရဲ့ တည်နေရာ (Index) ကို ရှာဖွေခြင်း ---
                int targetIndex = _categories.indexWhere((element) => element.id == _itemToModify!.id);
                
                if (targetIndex != -1) {
                  // မူလနေရာမှာပဲ တန်ဖိုးတွေကို တိုက်ရိုက် ပြောင်းလဲပေးခြင်း
                  _categories[targetIndex].name = _nameController.text;
                  _categories[targetIndex].color = _tempColor;
                  // အကယ်၍ Icon ကိုပါ ပြင်ဆင်ခွင့်ပေးချင်ရင် အောက်က line ကိုပါ ဖွင့်ပေးနိုင်ပါတယ်
                  // _categories[targetIndex].icon = _tempIcon; 
                }
                
                // ပြင်ဆင်ပြီးပါက Variable များကို Reset ချပြီး View State သို့ ပြန်သွားခြင်း
                _itemToModify = null;
                _currentState = CategoryState.view;
              }
            }),
              ...
              children: List.generate(navIcons.length, (index) {
                ...
                return SizedBox(
                  ...
                  child: GestureDetector(
                    behavior: HitTestBehavior.translucent,
                    onTap: () async {
                      // Assignment Icon (Index 3) ကို နှိပ်လိုက်လျှင် RecordHistoryScreen ဆီ ကူးပြောင်းပေးခြင်း
                      if (index == 3) {
                        setState(() {
                          _currentTabIndex = index;
                        });
                        ...
                        // History Screen ကနေ ပြန်ထွက်လာရင် အလယ်က Category tab ဆီ Auto ပြန်ရွေးပေးခြင်း
                        setState(() {
                          _currentTabIndex = 2;
                          _currentState = CategoryState.view;
                        });
